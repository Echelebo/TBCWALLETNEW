...
class userController extends Controller
{
      ...
      if(Auth::attempt(['email' => $req['email'], 'password' => $req['password']])) 
      {
        ...
        if($user->status == 0 || $user->status == 'New' || $user->status == 'pending')
        {
            if(env('EMAIL_CONFIRM') == 1)
            {
              ...
              Session::put('regMsg', __('messages.act_nt_actvtd'));
              return redirect()->route('confirm_email'); //->withErrors(['msg', 'Account not activated']); 
            }            
        }
        ...
        {
          $act = new activities;
          $act->descr = __('messages.usr_logd_');
          $act->action = "login";
          $act->title = __('messages.usr_logd_');
          $act->user_id = $user->id;
          $act->save(); 
  ...
  {        
      ...
      {  
        try
        {
          ...
          $act = new activities;
          $act->descr = __('messages.upd_prf_pic');
          $act->action = "profile_update";
          $act->title = __('messages.usr_upt_ppic');
          $act->user_id = $user->id;
          $act->save();
            ...
            try
          {
                ...
                if(Hash::check($req->input('oldpwd'), $user->pwd))
                {
                  ...
                  $usr->save();
                      ...
                      $act = new activities;
                      $act->descr = __('messages.usr_chng_pwd2');
                      $act->title = __('messages.usr_chng_pwd');
      ...
      if(!empty($user))
      {
        try
        {
          $validate = $req->validate([
            ...
            $act = new activities;
            $act->descr = __('messages.prfl_upd_succ');
            $act->title = __('messages.usr_upt_ppic');
            $act->action = "profile_update";
            $act->user_id = $user->id;
            $act->save();
  ...
  {  
      ...
      {         
        ...
        {         
          ...
          if($req->input('bname') == "BTC" || $req->input('bname') == "ETH" || $req->input('bname') == "USDT")
          {
          ...
          $act = new activities;
          $act->descr = __('messages.usr_add_bnk_dtl2');
          $act->title = __('messages.usr_add_bnk_dtl');
          $act->action = "add_bank";
          $act->user_id = $user->id;
          $act->save();
        ...
        {
            Session::put('status', __('messages.err_savn_detl').$e->getMessage());
            Session::put('msgType', "err");
            return back();
        }                 
  ...
  {        
      ...
      {            
        ...
        {         
              ...
              $act = new activities;
              $act->action = "delete_bank_acct";
              $act->descr = __('messages.usr_delt_bnk');
              $act->title = __('messages.usr_delt_bnk2');
              $act->user_id = $user->id;
              $act->save();
    ...
    public function user_edit_bank(Request $req)
    {        
      $user = Auth::User();
      if(!empty($user))
      {            
      ...
      {            
        ...
        {     
          ...
          if($capital >= $pack->min && $capital <= $pack->max) 
          {
            ...
            $act = new activities;
            $act->user_id = $user->id;
            $act->descr = __('messages.usr_invtd')." ".$capital." in ".$pack->package_name." package";
            $act->title = __('messages.invst');
            $act->action = "invest";
            $act->amt = $capital;
            $act->save();
  ...
  {        
      ...
      {            
        ...
        { 
          ...
          $act = new activities;          
          $act->user_id = $user->id;
          $act->descr = __('messages.usr_wthdrwn') .$pack->package.__('messages.pkg_p_id').' '.$pack->id;
          $act->title = __('messages.usr_wthdrwn2');
          $act->action = "wd_invest";
          $act->amt = $amt;
          $act->save();
  ...
  {        
      ...
      {         
        ...
        {  
          ...
          $act = new activities;
          $act->user_id = $user->id;
          $act->descr = __('messages.usr_reqst');
          $act->title = __('messages.usr_reqst2');
          $act->action = "wallet_wd";
          $act->amt = intval($req->input('amt'));
          $act->save();
  ...
  {        
      ...
      { 
        ...
        {  
          ...
          $act = new activities;
          $act->user_id = $user->id;
          $act->descr = __('messages.usr_requst_bns');
          $act->title = __('messages.usr_requst_bns2');
          $act->action = "ref_bn_wd";
          $act->amt = intval($req->input('amt'));
          $act->save();
  ...
  {        
      ...
      {  
        ...
        { 
          ...
          $act = new activities;
          $act->user_id = $user->id;
          $act->descr = __('messages.usr_dpst_des') ." ".$user->currency.intval($req->input('p_amt')).' '.__('messages.tru_bnk_tr');
          $act->title = __('messages.usr_dpst_des2');
          $act->action = "deposit";
          $act->amt = intval($req->input('p_amt'));
          $act->save();
      ...
      if($req->input('event') == 'successful' && $req->input('txref') == Session::get('pay_ref'))
      {
          try
          {
            ...
            return redirect('/'.$user->username.'/dashboard');
              ...
              $act->action = 
              $act->user_id = $user->id;
              $act->descr =__('messages.usr_dpst_des')." ".$user->currency.intval($req->input('p_amt')).' '.__('messages.tru_flutter');
              $act->title =__('messages.usr_dpst_des2');
              $act->action = "deposit";
              $act->amt = intval($req->input('p_amt'));
              $act->save();
  ...
  {        
      ...
      if(strtolower($user->username) == strtolower($req->input('usn')))
      {
      ...
      try
      {
          ...
          $act = new activities;
          $act->user_id = $user->id;
          $act->descr = __('messages.usr_fnd')." ".$user->currency.intval($req->input('s_amt'))." to ".trim($req->input('usn'));
          $act->title =__('messages.usr_fnd2');
          $act->action = "send_fund";
          $act->amt = intval($req->input('s_amt'));
          $act->save();
  ...
  { 
    ...
    {            
      ...
      {         
        ...
        $act = new activities;
        $act->user_id = $user->id;
        $act->descr = __('messages.usr_btc');
        $act->title = __('messages.usr_btc2');
        $act->action = "add_btc_wallet";
  ...
  public function pay_btc_amt(Request $req){
    ...
    if(($req->input('amount')*$st->currency_conversion) < env('MIN_DEPOSIT'))
    {
  ...
  public function bank_deposit(Request $req){
    ...
    if(!empty($user))
    {
        if($req->input('c_amount') < env('MIN_DEPOSIT'))
        {
          return back()->With(['toast_msg' => __('messages.amt_mb_grt').' '.env('MIN_DEPOSIT').' '.$this->st->currency, 'toast_type' => 'err']);
        }
      
        if($req->hasFile('c_pop') )
        {
          $file = $req->file('c_pop'); 
          $path = strtotime(date('Y-m-d H:s:i')).'_'.$user->username.".jpg";
          $file->move(base_path().'/../img/pmethod/', $path);
        }
        else
        {
            return back()->With(['toast_msg' => __('messages.mbc_proof_err') .' ', 'toast_type' => 'err']);
        }
        ...
        try{
            ...
            $paymt->user_id = $user->id;
            $paymt->usn = $user->username;
            $paymt->amount = $req->input('c_amount');
            $paymt->currency = $st->currency;
            $paymt->account_name = $req->input('act_name');
            $paymt->account_no = $req->input('act_num');
            $paymt->bank = $req->input('b_name');
            $paymt->url = "http://".request()->getHost().'/img/pmethod/'.$path;
            $paymt->status = 0;
            $paymt->on_apr = 0;
            $paymt->pop = $path;
  ...
  public function c_deposit(Request $req){
    $user = Auth::User();
    if(!empty($user))
    {
      try{
        ...
        $st = site_settings::find(1);
        if(($req->input('c_amount')* $st->currency_conversion) < env('MIN_DEPOSIT'))
        {
            return back()->With(['toast_msg' => __('messages.amt_mb_grt').' '.env('MIN_DEPOSIT').' '.$this->st->currency, 'toast_type' => 'err']);
        }
        ...
        if($req->hasFile('c_pop') )
        {
          $file = $req->file('c_pop'); 
          $path = strtotime(date('Y-m-d H:s:i')).'_'.$user->username.".jpg";
          $file->move(base_path().'/../img/pmethod/', strtotime(date('Y-m-d H:s:i')).'_'.$user->username.".jpg");
        }
        else
        {
            return back()->With(['toast_msg' => __('messages.mbc_proof_err') .' ', 'toast_type' => 'err']);
        ...
        $paymt->user_id = $user->id;
        $paymt->usn = $user->username;
        $paymt->amount = $req->input('c_amount')*$st->currency_conversion;
        $paymt->currency = $st->currency;
        $paymt->account_name = $req->input('c_name');
        $paymt->account_no = $req->input('c_wallet');
        $paymt->bank = $req->input('c_name');
        $paymt->url = "http://".request()->getHost().'/img/pmethod/'.$path;
        $paymt->status = 0;
        $paymt->on_apr = 0;
        $paymt->pop = $path;
  ...
  public function set_2fa($opr)
  {
    ...
    if($opr == 'enable')
    {
      ...
      $data_2fa = ['QR_Image' => '<img src="'.$QR_Image.'"/>', 'secret' => $secret_2fa, 'msg' => 'suc'];
      return json_encode($data_2fa);
      // dd($QR_Image);
    }
    elseif($opr == 'disable')
  ...
  public function rpay_amt(Request $req)
  {
     return view('user.rpay');
  }
  
  public function rpay_confirm(Request $req)
  {
    $user = Auth::User();
    if(!empty($user))
    {
        ...
        try{
            ...
            $paymt->user_id = $user->id;
            $paymt->usn = $user->username;
            $paymt->amount = $req->input('amt')*env('CONVERSION');
            $paymt->currency = $st->currency;
            $paymt->account_name = "Razorpay";
            $paymt->account_no = $req->input('razorpay_payment_id');
            $paymt->bank = 'Razorpay';
            $paymt->url = '';
            $paymt->status = 1;
            $paymt->on_apr = 1;
            ...
            $user->wallet += intval($req->input('amt')*env('CONVERSION'));
            $user->save();
