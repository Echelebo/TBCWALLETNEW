<table class="display table table-stripped table-hover">
    <thead>
        <tr>                       
            <th> {{ __('messages.adm_name') }} </th>
            <th> {{ __('messages.admin_frm_email') }} </th>
            <th> {{ __('messages.rol') }} </th>
            <th> {{ __('messages.sts') }} </th>                        
            <th> {{ __('messages.mang') }} </th>                                                                          
        </tr>
    </thead>
    <tbody>        
        @if(count($adm_users) > 0 )
            @foreach($adm_users as $dep)
                <tr> 
                    <td>{{$dep->name}}</td>
                    <td>{{$dep->email}}</td>
                    <td>
                        @if($dep->role == 1)
                        {{$dep->role = "Support"}}
                        @elseif($dep->role == 2)
                        {{$dep->role = "Manager"}}
                         @else
                         {{$dep->role = "Admin"}}
                        @endif
                    </td> 
                    <td>
                        @if($dep->status == 0)
                            {{ __('messages.pending') }}
                        @elseif($dep->status == 1)
                            {{ __('messages.sts_active') }}
                        @elseif($dep->status == 2)
                            {{ __('messages.blck') }}
                        @endif
                    </td>                   
                    <td>
                        <div class="dropdown show">
                            <a href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <i class="fa-solid fa-circle-ellipsis fa-2x text-primary"></i>
                            </a>
                            @if($adm->role == 3)
                                <div class="dropdown-menu " aria-labelledby="dropdownMenuLink">
                                    <a class="dropdown-item" title="Activate User" href="/admin/activate/admuser/{{$dep->id}}" > 
                                        <i class="fa fa-check text-success"></i> Activate
                                    </a>
                                    <a class="dropdown-item " title="Block User" href="/admin/ban/admuser/{{$dep->id}}" > 
                                        <i class="fa fa-ban text-warnig" ></i> Block
                                    </a>
                                    <a class="dropdown-item" title="Delete User" href="/admin/delete/admuser/{{$dep->id}}" > 
                                        <i class="fa fa-times text-danger"></i> Delete
                                    </a>
                                </div>
                            @endif 
                        </div>
                    </td>
                                   
                </tr>
            @endforeach
        @else
            <tr>                                                            
                <td>{{ __('messages.no_data') }} </td>                                        
            </tr>
        @endif
    </tbody>
</table>
      