...
function sel_pm(id, lnk){
    $('.p_method_sel').removeClass('pyt_method_sel');
    $('#'+id).addClass('pyt_method_sel');
    $('#pay_btn').attr('href', lnk);
}

function wd_sel_mth(id, lnk){
    $('.p_method_sel').removeClass('pyt_method_sel');
    $('#'+id).addClass('pyt_method_sel');
    $('#wd_form_page').attr('action', lnk);
    // $('#wd_switch').val(lnk);
    $('#wd_page_sbtn').removeClass('d-none');
    // alert('here');
}

$('#pm_bank_pop').on('click', function(){
    // alert('here');
    $('#pm_bank_modal').click();
})

function sel_crypto(id)
{
    if(id == "wal_btc")
    {
        $('#crypto_type').val('BTC');
    ...
    if(id == "wal_bch")
    {
        $('#crypto_type').val('USDT');
    }
}

function copy_txt() {
    var text = document.getElementById('reflnk').innerText;
    ...
    $('#suc').html("Referral link copied");            
    $('#suc').show().animate({ width: "50%" }, "1000").delay(2000).fadeOut(1000);
} 
...
function set_inputs(num)
{
    ...
    for(i = 1; i<=num; i++)
    {
        div_str = div_str+'<div class="row mt-5 ">\
...
function toggle_ref_sett(id)
{
	if($("#"+id).val() == 'Multi_level')
	{
		$("#"+id).val('Single_level');
